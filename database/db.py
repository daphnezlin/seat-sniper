...
async def get_watchers(course_code: str, term: str):
    p = await get_pool()
    rows = await p.fetch(
        "SELECT user_phone, email, notify_method FROM watched_courses WHERE course_code = $1 AND term = $2 AND active = true",
        course_code, term
    )
    return [dict(row) for row in rows]
...
async def add_watcher(phone: str, email: str, notify_method: str, course_code: str, term: str):
    p = await get_pool()
    await p.execute(
        "INSERT INTO watched_courses (user_phone, email, notify_method, course_code, term) VALUES ($1, $2, $3, $4, $5)",
        phone, email, notify_method, course_code, term
    )

async def log_notification(contact: str, course_code: str, section: str, message: str):
    p = await get_pool()
    await p.execute(
        "INSERT INTO notifications (user_phone, course_code, section, message) VALUES ($1, $2, $3, $4)",
        contact, course_code, section, message
    )